...
with open('One_hot_encoder_geography.pkl', 'rb') as file:
    one_hot_encoder_geography = pickle.load(file)  # Corrected variable name
...
input_data = pd.DataFrame({
    ...
    'Gender': [label_encoder_gender.transform([gender])[0]],  # Fixed transformation
    'Age': [age],
    'Tenure': [tenure],
    'Balance': [balance],
...
# OHE Geography
geo_encoded = one_hot_encoder_geography.transform([[geography]]).toarray()  # Fixed OneHotEncoder transformation
geo_encoded_df = pd.DataFrame(geo_encoded, columns=one_hot_encoder_geography.get_feature_names_out(['Geography']))

# Combine OHE with input
